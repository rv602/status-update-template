    ...
    $.extend($.summernote.plugins, {
        imageAttributes: function(context) {
            ...
            if (options.maximumImageFileSize) {
                ...
                imageAttributesLimitation =
                    '<small class="help-block note-help-block">' + lang.image.maximumFileSize + ' : ' + readableSize + '</small>';
            }
            ...
            this.initialize = function() {
                ...
                body +=
                    ...
                    // '      <label class="control-label note-form-label col-sm-3">' +
                    // lang.imageAttributes.style +
                    // '</label>' +
                    // '      <div class="input-group note-input-group col-xs-12 col-sm-9">' +
                    ...
                    '      <label class="control-label note-form-label col-xs-3">' +
                    lang.imageAttributes.linkHref +
                    '</label>' +
                    '      <div class="input-group note-input-group col-xs-12 col-sm-9">' +
                    ...
                    // '      <label class="control-label note-form-label col-xs-3">' +
                    // lang.imageAttributes.linkStyle +
                    // '</label>' +
                    // '      <div class="input-group note-input-group col-xs-12 col-sm-9">' +
                ...
                if (options.imageAttributes.disableUpload == false) {
                    // Tab 4
                    body +=
                        '  <div class="tab-pane note-tab-pane" id="note-imageAttributes-upload' +
                        timestamp +
                        '">' +
                        '   <label class="control-label note-form-label col-xs-3">' +
                        lang.imageAttributes.upload +
                        '</label>' +
                        '   <div class="input-group note-input-group col-xs-12 col-sm-9">' +
                ...
                // Tab 1
                body +=
                    '  <div class="tab-pane note-tab-pane fade in active" id="note-imageAttributes' +
                    timestamp +
                    '">' +
                    '    <div class="note-form-group form-group note-group-imageAttributes-url">' +
                    '      <label class="control-label note-form-label col-sm-3">' +
                    lang.imageAttributes.src +
                    '</label>' +
                    '      <div class="input-group note-input-group col-xs-12 col-sm-9">' +
                    ...
                    '      <label class="control-label note-form-label col-sm-3">' +
                    lang.imageAttributes.dimensions +
                    '</label>' +
                    '      <div class="input-group note-input-group col-xs-12 col-sm-9">' +
                ...
                this.$dialog = ui
                    .dialog({
                        ...
                        body: body,
                        footer:
                            '<button href="#" class="btn btn-primary note-btn note-btn-primary note-imageAttributes-btn">' +
                            lang.imageAttributes.editBtn +
                            '</button>'
                    })
                    .render()
                    .appendTo($container);
            ...
            this.show = function() {
                ...
                this.showImageAttributesDialog(imgInfo).then(function(imgInfo) {
                    ...
                    if (options.imageAttributes.removeEmpty) {
                        if (imgInfo.alt) $img.attr('alt', imgInfo.alt);
                        else $img.removeAttr('alt');
                        if (imgInfo.width) $img.attr('width', imgInfo.width);
                        else $img.removeAttr('width');
                        if (imgInfo.height) $img.attr('height', imgInfo.height);
                        else $img.removeAttr('height');
                        if (imgInfo.title) $img.attr('title', imgInfo.title);
                        else $img.removeAttr('title');
                        if (imgInfo.src) $img.attr('src', imgInfo.src);
                        else $img.attr('src', '#');
                        if (imgInfo.class) $img.attr('class', imgInfo.class);
                        else $img.removeAttr('class');
                        if (imgInfo.style) $img.attr('style', imgInfo.style);
                    ...
                    let parsedAnchorTag = anchorme(imgInfo.linkHref);
                    var wrapper = document.createElement('div');
                    wrapper.innerHTML = parsedAnchorTag;
                    parsedAnchorTag = wrapper.firstChild;
                    let parsedURL = parsedAnchorTag && parsedAnchorTag.href;
                    if (imgInfo.linkHref) {
                        var linkBody = '<a';
                        if (imgInfo.linkClass) linkBody += ' class="' + imgInfo.linkClass + '"';
                        if (imgInfo.linkStyle) linkBody += ' style="' + imgInfo.linkStyle + '"';
                        linkBody += ' href="' + (parsedURL || imgInfo.linkHref) + '" target="' + imgInfo.linkTarget + '"';
