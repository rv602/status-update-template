...
(function (global, factory) {
	...
	function minMaxTimePlugin (config) {
		...
		function findDateTimeLimit (date, flatpickrInstance) {
			if (config.table !== undefined) {
				return config.table[state.formatDate(date, state.tableDateFormat)];
			}

			return config.getTimeLimits && config.getTimeLimits(date, flatpickrInstance);
		}
		...
		return function (fp) {
			return {
				...
				onChange: function onChange () {
					...
					var matchingTimeLimit = latest && findDateTimeLimit(latest, this);
